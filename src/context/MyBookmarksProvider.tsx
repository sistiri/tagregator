...
import MyBookmarksContext, {
  MyBookmarksContextObj,
} from "./myBookmarks-context";
...
const MyBookmarksProvider: React.FC<MyBookmarksProviderProps> = (props) => {
  ...
  const { sendRequest: fetchDeleteBookmarks } = useHttp();
  
  const removeBookmarkHandler = (id: string) => {
    setMyBookmarks((prevBookmarks) =>
      prevBookmarks.filter((bm) => bm.id !== id)
    );
    fetchDeleteBookmarks({
      url: `https://tagregatory-default-rtdb.europe-west1.firebasedatabase.app/bookmarks/${id}.json`,
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    }, 
    (id: string) => setMyBookmarks(prevBookmarks =>
      prevBookmarks.filter((bm) => bm.id !== id)));
  };
